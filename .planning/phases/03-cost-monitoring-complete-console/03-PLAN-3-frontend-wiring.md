...
must_haves:
  ...
  artifacts:
    - path: "frontend/src/api/client.js"
      provides: "api.spend(params) method"
      contains: "spend"
    ...
    - path: "frontend/src/components/NavBar.vue"
      provides: "Cost adminLink with reactive badge count"
    - path: "frontend/src/router/index.js"
      provides: "/cost route pointing to CostView"
  key_links:
    - from: "frontend/src/components/NavBar.vue"
      to: "/admin/spend"
      via: "api.spend() call on mount to get alertCount"
      pattern: "api.spend"
...
<objective>
Wire all frontend infrastructure needed by CostView before building the view itself: add `api.spend()` to the API client, extend StatusBadge with the three new budget statuses (ok/warning/over), add the "Cost" admin nav link with its reactive badge to NavBar, and register the `/cost` route.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Zero / falsy filter values are omitted from query string (not sent as ?team_id=0)
 ...
 *   from/to: YYYY-MM-DD strings. teamId/appId/keyId: numeric IDs for filtering.
 *   Omit any param to use defaults (server uses last 7 days if from/to not provided).
 */
spend(params = {}) {
  const qs = new URLSearchParams()
  ...
  if (params.keyId) qs.set('key_id', String(params.keyId))
...
Note: The existing `client.test.js` tests cover the `request` helper. No changes to client.test.js are needed — `spend()` follows the exact same pattern as `logs()`.
  ...
  <done>npm test exits 0. No existing client.test.js tests broken. api.spend is exported from client.js.</done>
</task>
...
<task type="auto" tdd="true">
  <name>Task 3: Add Cost link + reactive badge to NavBar + register /cost route</name>
  <files>
    frontend/src/components/NavBar.vue
    frontend/src/router/index.js
    frontend/tests/unit/components/NavBadge.test.js
  ...
  <behavior>
    ...
    - /cost route in router/index.js points to CostView component (lazy-import is fine)
...
1. Add `onMounted` to imports from 'vue': `import { onMounted, ref } from 'vue'`
2. Add `api` import (already imported from client.js — verify it's there)
3. Add `alertCount` ref: `const alertCount = ref(0)`
4. Add `onMounted` to fetch alert count:
...
5. Add `{ to: '/cost', label: 'Cost' }` to `adminLinks` after the Keys entry.

6. The Cost link in the template needs special treatment for the badge. Replace the generic `v-for="link in adminLinks"` router-link block with a version that handles the Cost link badge. The simplest approach: render adminLinks with the existing v-for, but add a conditional badge span for the Cost link only:
...
1. Add CostView import at the top (after KeysView):
```javascript
import CostView from '../views/CostView.vue'
```

2. Add route to the `routes` array (after the keys route):
```javascript
{ path: '/cost', name: 'cost', component: CostView },
```

Note: CostView.vue does not exist yet — it will be created in Plan 4. This will cause a build error if CostView is statically imported and the file doesn't exist. To avoid this, use a dynamic import instead:
```javascript
{ path: '/cost', name: 'cost', component: () => import('../views/CostView.vue') },
```
Dynamic import defers the module resolution — the router compiles even if the file doesn't exist yet, and the error only occurs at route navigation time (which won't happen until Plan 4 creates the file).

**Update `frontend/tests/unit/components/NavBadge.test.js`:**

Replace the `it.todo` stubs with real tests. Mount NavBar with a mocked `api.spend()`:
...
describe('NavBar Cost badge', () => {
  ...
  it('renders numeric badge when alertCount > 0', async () => {
    ...
    await flushPromises()
    // Check that a badge span exists (alertCount = 1)
...
If NavBar tests are too brittle due to useSession coupling, it's acceptable to test at a more limited scope (just verify the badge renders in isolation) and mark the full NavBar integration test as `it.skip` with a comment. The CRITICAL assertion is that `StatusBadge.test.js` passes (tests the component that handles ok/warning/over statuses).
  ...
  <done>StatusBadge tests all pass. NavBadge tests pass (or are explicitly skipped with comment if useSession coupling makes them too brittle). /cost route exists in router/index.js. NavBar adminLinks contains Cost entry.</done>
</task>
